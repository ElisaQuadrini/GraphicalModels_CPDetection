...
# L_true = 3 labelled clusters, but only 2 distinct regimes: clusters 1 and 3
# share the same graph, precision matrix, and mean vector. Cluster 2 is the
# only genuinely distinct regime. This tests whether the collapsed sampler
# recovers L = 2 underlying regimes rather than being misled by the
# arbitrary 3-way true-label split.

p      <- 5       # number of variables
n      <- 150     # total number of observations
n1     <- 50      # obs per cluster (balanced for simulation)
L_true <- 3        # number of true LABELS (not regimes)

# regime A graph: two cliques {1,2,3} and {3,4,5} -- shared by clusters 1 and 3
AdjA <- matrix(0, p, p)
AdjA[1:3, 1:3] <- 1
AdjA[3:5, 3:5] <- 1
diag(AdjA) <- 0

# regime B graph: cliques {1,2} and {2,3,4,5} -- used only by cluster 2
AdjB <- matrix(0, p, p)
AdjB[1:2, 1:2] <- 1
AdjB[2:5, 2:5] <- 1
diag(AdjB) <- 0
...
Adj1 <- AdjA
Adj2 <- AdjB
Adj3 <- AdjA   # cluster 3 uses the same graph as cluster 1

b_gen <- 3
D_gen <- diag(p)

# draw the precision matrix once per REGIME, not once per label
OmegaA_true <- rgwish(n = 1, adj = AdjA, b = b_gen, D = D_gen)
OmegaB_true <- rgwish(n = 1, adj = AdjB, b = b_gen, D = D_gen)

Omega1_true <- OmegaA_true
Omega2_true <- OmegaB_true
Omega3_true <- OmegaA_true   # same precision matrix as cluster 1
...
mu1_true <- muA_true
mu2_true <- muB_true
mu3_true <- muA_true   # same mean as cluster 1
...
xi_true <- rep(1:L_true, each = n1) # true LABEL vector (3 labels)
...
tryCatch({
  pdf("figures/trace_L_alpha0_clustering_sharedregime.pdf", width = 10, height = 8)
...
tryCatch({
  pdf("figures/coclustering_matrix_clustering_sharedregime.pdf", width = 7, height = 7)
...
# =============================================================================
# NOTE: loop over the ESTIMATED clusters actually present in xi_estimated,
# not over L_true. Since clusters 1 and 3 share the same regime, the
# posterior mode of L (and hence xi_estimated) may collapse to fewer than
# L_true distinct labels -- looping over 1:L_true would hit empty groups.

est_labels <- sort(unique(xi_estimated))
L_est      <- length(est_labels)
...
cat("Any NA in X_rep after rebuilding?", sum(is.na(X_rep)), "\n")

# 7.1: Compute Observed vs Replicated Cluster-Specific Statistics
t_obs_mean_clust <- matrix(NA, L_est, p)
t_obs_var_clust  <- matrix(NA, L_est, p)

for (idx in seq_along(est_labels)) {
  l <- est_labels[idx]
  X_cluster_real <- X[xi_estimated == l, , drop = FALSE]
  t_obs_mean_clust[idx, ] <- colMeans(X_cluster_real)
  t_obs_var_clust[idx, ]  <- apply(X_cluster_real, 2, var)
}

t_rep_mean_clust <- array(NA, c(S_ppc, L_est, p))
t_rep_var_clust  <- array(NA, c(S_ppc, L_est, p))
...
for (s in 1:S_ppc) {
  for (idx in seq_along(est_labels)) {
    ...
    if (length(in_l_rep) > 1) {
      t_rep_mean_clust[s, idx, ] <- colMeans(X_rep[s, in_l_rep, ])
      t_rep_var_clust[s, idx, ]  <- apply(X_rep[s, in_l_rep, ], 2, var)
    } else {
      t_rep_mean_clust[s, idx, ] <- rep(NA, p)
      t_rep_var_clust[s, idx, ]  <- rep(NA, p)
    }
  }
}
...
tryCatch({
  pdf("figures/ppc_means_per_cluster_sharedregime.pdf", width = 3 * L_est, height = 2.5 * p)

  par(mfrow = c(p, L_est), mar = c(3, 3, 2.5, 1), oma = c(1, 1, 4, 0))
  for (j in 1:p) {
    for (idx in seq_along(est_labels)) {
      l <- est_labels[idx]
      rep_vals <- t_rep_mean_clust[, idx, j]
      ...
      abline(v = t_obs_mean_clust[idx, j], col = "red", lwd = 2)
    }
  }
...
tryCatch({
  pdf("figures/ppc_variances_per_cluster_sharedregime.pdf", width = 3 * L_est, height = 2.5 * p)

  par(mfrow = c(p, L_est), mar = c(3, 3, 2.5, 1), oma = c(1, 1, 4, 0))
  for (j in 1:p) {
    for (idx in seq_along(est_labels)) {
      l <- est_labels[idx]
      rep_vals <- t_rep_var_clust[, idx, j]
      rep_vals <- rep_vals[!is.na(rep_vals)]
      ...
      abline(v = t_obs_var_clust[idx, j], col = "red", lwd = 2)
    }
  }
...
p_mean_tbl <- sapply(seq_along(est_labels), function(idx) sapply(1:p, function(j) {
  rep_vals <- t_rep_mean_clust[, idx, j]
  bayes_p_value(t_obs_mean_clust[idx, j], rep_vals[!is.na(rep_vals)])
}))
p_var_tbl <- sapply(seq_along(est_labels), function(idx) sapply(1:p, function(j) {
  rep_vals <- t_rep_var_clust[, idx, j]
  bayes_p_value(t_obs_var_clust[idx, j], rep_vals[!is.na(rep_vals)])
}))

colnames(p_mean_tbl) <- paste0("Cluster", est_labels)
colnames(p_var_tbl)  <- paste0("Cluster", est_labels)
...
# Ground truth is now regime_true (2 distinct regimes), not xi_true (3 labels),
# since clusters 1 and 3 were generated from the same regime.
# =============================================================================

conf_mat  <- table(Estimated = xi_estimated, True = regime_true)
cost_mat  <- max(conf_mat) - conf_mat
perm_lsap <- solve_LSAP(t(cost_mat))

cat("\nOptimal cluster alignment (true regime k -> estimated label perm[k]):\n")
for (k in seq_len(ncol(conf_mat))) {
  cat(sprintf("  True regime %d  ->  Estimated label %d\n", k, perm_lsap[k]))
}
...
# 8.1: Compute Per-Cluster Posterior Inclusion Probabilities (PIP) -- unchanged
...
# only 2 distinct regimes now
Adj_list <- list(AdjA, AdjB)

# 8.2: Save PIP vs True Adjacency Heatmaps to PDF
tryCatch({
  pdf("figures/graph_recovery_pip_heatmaps_clustering_sharedregime.pdf", width = 8, height = 3.5 * L_hat_val)
  ...
  for (k in seq_len(L_hat_val)) {
    ...
    if (true_k >= 1 && true_k <= length(Adj_list)) {
      image(1:p, 1:p, t(Adj_list[[true_k]]),
            ...
            main = paste("True Regime", true_k, "- True Adjacency"),
            xlab = "Node Index", ylab = "Node Index")
    } else {
      plot.new()
      title("No True Regime Matched")
    }
  }
...
cat("\nGraph recovery metrics per estimated cluster:\n")
for (k in seq_len(L_hat_val)) {
  true_k <- est_to_true[k]
  if (true_k >= 1 && true_k <= length(Adj_list)) {
    metrics_k <- graph_recovery_metrics(pip_cluster[, , k], Adj_list[[true_k]])
    cat(sprintf("Estimated Cluster %d <-> True Regime %d:\n", k, true_k))
    print(metrics_k)
  }
}
