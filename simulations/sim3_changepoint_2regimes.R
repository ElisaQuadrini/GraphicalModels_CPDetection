...
# Illustration variant: block 1 and block 3 are generated from the SAME regime
# (same graph, same precision matrix, same mean), while block 2 is the only
# genuinely distinct regime. Since blocks are temporally ordered and the
# restricted EPPF never merges non-adjacent segments, this tests whether the
# sampler still detects BOTH change-points, even though the third block
# reverts to the same dependence structure as the first.
...
# performed to preserve the temporal/sequential structure.
...
Adj1 <- AdjA
Adj2 <- AdjB
Adj3 <- AdjA   # block 3 uses the same graph as block 1
...
Omega3_true <- OmegaA_true   # same precision matrix as block 1

# True means: block 3 shares the mean of block 1 as well
muA_true <- c( 2,  2,  0, -1, -1)
muB_true <- c(-2,  0,  1,  1,  0)

mu1_true <- muA_true
mu2_true <- muB_true
mu3_true <- muA_true   # same mean as block 1
...
tryCatch({
  pdf("figures/changepoint_mcmc_diagnostics_sharedregime.pdf", width = 10, height = 8)
...
tryCatch({
  pdf("figures/changepoint_psm_matrix_sharedregime.pdf", width = 7, height = 7)
...
# NOTE: the true adjacency for block 3 is now AdjA (same regime as block 1),
# not a distinct graph -- Adj3 already reflects this from Section 1.
...
tryCatch({
  pdf("figures/changepoint_graph_recovery_pip_sharedregime.pdf", width = 10, height = 7)
  
  par(mfrow = c(2, 3))
  plot_matrix(Adj1, "True Graph: Block 1 (Regime A)")
  plot_matrix(Adj2, "True Graph: Block 2 (Regime B)")
  plot_matrix(Adj3, "True Graph: Block 3 (Regime A)")
...
tryCatch({
  pdf("figures/changepoint_probabilities_plot_sharedregime.pdf", width = 9, height = 5)
...
cat("NOTE: block 1 and block 3 share the same regime; a well-calibrated\n")
cat("model should still detect both change-points, since they mark\n")
cat("temporal transitions, not distinguishable long-run regimes.\n")
...
#    even though blocks 1 and 3 share the same regime)
...
tryCatch({
  pdf("figures/changepoint_ppc_means_sharedregime.pdf", width = 3 * K_true, height = 2.5 * p)
...
tryCatch({
  pdf("figures/changepoint_ppc_variances_sharedregime.pdf", width = 3 * K_true, height = 2.5 * p)
