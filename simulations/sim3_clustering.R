...
# Adjusted Rand Index (shared with sim2_changepoint.R via compute_ari)
ari_value <- compute_ari(xi_estimated, xi_true)
cat(sprintf("\nAdjusted Rand Index (estimated vs true partition): %.3f\n", ari_value))
...
# 7.1 CLUSTER-SPECIFIC STATISTICS (OBSERVED VS REPLICATED)
# Observed statistics are grouped by xi_estimated so they align with the
# replicated statistics, which are grouped by the same fixed partition.
...
par(mfrow = c(p, L_true), mar = c(2.5, 2.5, 2, 1), oma = c(1, 1, 3, 0))
for (j in 1:p) {
  for (l in 1:L_true) {
    ...
    hist(rep_vals,
         ...
         main = paste("Cl", l, "- Var", j),
         xlab = "", ylab = "", cex.main = 0.9)

    abline(v = t_obs_mean_clust[l, j], col = "red", lwd = 2)
  }
}
title("Posterior Predictive Checks: MEANS per Cluster and Variable", outer = TRUE, cex.main = 1.3)
...
title("Posterior Predictive Checks: VARIANCES per Cluster and Variable", outer = TRUE, cex.main = 1.3)

# Bayesian p-values summary table (shared bayes_p_value from R/validation_checks.R)
...
# Uses the Hungarian algorithm (solve_LSAP from package 'clue') to find the
# optimal bijective mapping between estimated and true cluster labels.
#
# Reference: Kuhn (1955), Naval Research Logistics Quarterly;
#            Hornik (2005), clue: Cluster Ensembles, R package.
# =============================================================================

# -----------------------------------------------------------------------------
# 8.1: Build confusion matrix and solve the optimal label assignment
# Both xi_estimated and xi_true are in the same (shuffled) observation order.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 8.2: Per-cluster PIP - accumulate over post-burnin iterations, mapping the
# current MCMC label back to the estimated cluster index via a majority vote
# over observations that xi_estimated assigned to each cluster k.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 8.3: True adjacency matrices (same order as simulation)
# -----------------------------------------------------------------------------

Adj_list <- list(Adj1, Adj2, Adj3)

# -----------------------------------------------------------------------------
# 8.4: Heatmaps - PIP vs true adjacency, side by side for each cluster.
# -----------------------------------------------------------------------------

par(mfrow = c(L_hat_val, 2), mar = c(2, 2, 3, 1))
...
for (k in seq_len(L_hat_val)) {
  ...
  image(1:p, 1:p, t(pip_cluster[, , k]),
        zlim = c(0, 1),
        col  = heat.colors(100, rev = TRUE),
        main = paste("Estimated cluster", k, "- PIP"),
        xlab = "node", ylab = "node")
  ...
  if (true_k >= 1 && true_k <= length(Adj_list)) {
    image(1:p, 1:p, t(Adj_list[[true_k]]),
          ...
          main = paste("True cluster", true_k, "- Adjacency"),
          xlab = "node", ylab = "node")
  } else {
    plot.new()
...
# -----------------------------------------------------------------------------
# 8.5: Graph recovery metrics (precision, recall, F1, Hamming distance),
# using the shared graph_recovery_metrics() from R/validation_checks.R -
# consistent with sim1_base_model.R and sim2_changepoint.R.
# -----------------------------------------------------------------------------
